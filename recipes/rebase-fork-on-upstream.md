...
- **Move to…** dialog (session row menu) → moved `dummy1 → dummy2`: log dir
  ...
  usable and the next turn ran cold in `/Users/USER/projects/dummy2`.
...
| `<private-plugin>` (found 2026-09-21, **open**) | Settings ▸ Plugins ▸ "<private> kernel" card never appears (no error: `slots.inject` waits for a slot that is never declared) | The `settings.plugin.item` keyed slot is gone; ui-settings-plugins now declares `settings.plugins.tab` (list) — a different contract. Needs a port of `kernel-card.tsx`'s registration. Found by `pnpm typecheck` in the plugin. |
