...
(`https://laptop.example.ts.net/dsh/`) with the from-scratch
...
- The fork's route was **port-based** (`http://desk/`) because a `/dsh`
...
- `Host` stays the tailnet FQDN (`laptop.example.ts.net`);
...
- This node: `laptop.example.ts.net`, tags `tag:research`,
...
QR and got the DSH GUI at `https://laptop.example.ts.net/dsh/`.
...
2. Settings → **Tailscale remote** → **Enable**. Copy
   `https://laptop.example.ts.net/dsh/` or scan the QR from the
