...
`https://laptop.example.ts.net/dsh/`, is admitted by this Mac's
...
1. **This node is not tagged** (`tailscale status --self --json`: `Tags: null`,
   user `user@example.com`), and a request the node makes to *itself*
   through Serve carries `tailscale-user-login: user@example.com` and
...
```
Dock app / phone ─▶ https://laptop.example.ts.net/dsh/
...
1. `~/.dsh/tailscale-remote.json`: `allowedUsers: ["user@example.com"]`
...
2. `~/.dsh/profiles/web/cordis.patch.yml`, row `tali-tailscale-remote`:
   ...
   config:
     listenPort: 3084
     publishPort: 3083
     relayCwd: /Users/USER/github/deepseek-harness
...
- id: tali-tailscale-remote
  config: { instance: preview, listenPort: 3086, publishPort: 3085, mountPath: /dsh-preview,
            ...
            relayCwd: /Users/USER/github/deepseek-harness,
            relayStart: pnpm dsh --profile web --patch /Users/USER/github/tali-dash-plugins/cordis.dev.yml --no-open --port 3088 }
```
...
at `https://laptop.example.ts.net/dsh/` and `/dsh-preview/`, with
