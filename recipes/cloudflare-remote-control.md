...
Recipe for driving the DeepSeek Harness Web GUI running on the laptop (`desk`)
...
pnpm add -w link:/Users/USER/github/dsh-full-remote
...
*Tunnel target* (`http://desk/`) which also becomes the default invite
...
- **Port-based route, not a `/dsh` path.** The original spec wanted
  `http://desk/dsh`. Not viable: tailscaled strips the `--set-path` mount
  ...
  would 404 at `http://desk/assets/...`. Rewriting minified bundles was
  judged too fragile. Route = a port on the node: `80` → `http://desk/`,
  `8080` → `http://desk:8080/`. Default HTTP on 80 (short MagicDNS name;
  WireGuard encrypts in transit). HTTPS is available but must use the FQDN
  `https://desk.example.ts.net/` — the Tailscale cert only covers that.
  DSH's client needs no secure context (`randomUUID` is built on
  `getRandomValues`), so plain `http://` works on iOS.
- **Explicit login allowlist, not "== local user".** `desk` is a *tagged*
  node (`tag:research`, `tag:typst-host`) on the `example.com` tailnet, so it
  has no owning user (`tailscale status --json` reports `Self.UserID` →
  `LoginName: desk.example.ts.net`), and no `user@example.com` user
  exists until the phone joins. The allowlist must be typed:
  `user@example.com` (whatever SSO login the phone ends up with; check the
...
`http://desk:8099/probe` answered HTTP 200 through tailscaled with
`x-forwarded-for: 100.114.226.21` (the node's own tailnet IP, in the CGNAT
range) and `x-forwarded-host: desk:8099`, no identity header (self-probe from
...
1. Start proxy → **Tailscale → Enable route** (Protocol HTTP, Port 80).
   Card shows *Route is active*, hint `Open http://desk/ from any device on
   your tailnet.`; **Run self-check** should report HTTP 200.
2. Phone on the tailnet: either scan an invite (its origin is now
   `http://desk/`) — or tick **User-based auth**, enter `user@example.com`,
   **Save logins**, and just open `http://desk/` — no token, no invite.
...
| `tailscale serve` on the company tailnet | works technically (`dsh web --trusted-host desk.example.ts.net`) but the phone would have to join the company tailnet |
