...
#
#   pnpm build-app [--skip-pack] [--no-dmg] [--name DSH] [--port 3090] [--sign IDENTITY]
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --no-dmg|--name|--port|--sign|--version) if [[ "$1" == --no-dmg ]]; then APP_ARGS+=("$1"); shift; else APP_ARGS+=("$1" "$2"); shift 2; fi ;;
