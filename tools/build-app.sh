...
#
#   pnpm build-app [--skip-pack] [--no-dmg] [--no-prune] [--with-office] [--name DSH] [--port 3090] [--sign IDENTITY]
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --no-dmg|--no-prune|--with-office|--name|--port|--sign|--version) if [[ "$1" == --no-dmg || "$1" == --no-prune || "$1" == --with-office ]]; then APP_ARGS+=("$1"); shift; else APP_ARGS+=("$1" "$2"); shift 2; fi ;;
