...
describe('server pane helpers', () => {
  ...
  it('labels user agents', () => {
    assert.equal(describeUserAgent('Mozilla/5.0 (Macintosh) AppleWebKit/605 (KHTML, like Gecko) DSHDock/1.0'), 'DSH Dock app')
    assert.equal(describeUserAgent('Mozilla/5.0 (iPhone; CPU iPhone OS 17_0) AppleWebKit/605 Version/17.0 Mobile/15E148 Safari/604.1'), 'Safari (iOS)')
  ...
  it('reads identity facts from the proxy headers or the socket', () => {
    const proxied = clientFacts({ headers: { 'x-dsh-tailscale-remote': '1', 'x-dsh-tailscale-remote-admitted': 'user', 'x-dsh-tailscale-remote-login': 'user@example.com', 'x-forwarded-for': '100.1.2.3', 'x-dsh-tailscale-remote-self': '1', 'user-agent': 'DSHDock/1.0' }, socket: { remoteAddress: '127.0.0.1' } })
    assert.deepEqual(proxied, { proxied: true, login: 'user@example.com', admitted: 'user', address: '100.1.2.3', self: true, userAgent: 'DSHDock/1.0' })
...
describe('client tracker on a live http.Server', () => {
  ...
  it('records requests, sniffs the session body without consuming it, and counts live sockets', async () => {
    const proxied = { 'x-dsh-tailscale-remote': '1', 'x-dsh-tailscale-remote-admitted': 'user', 'x-dsh-tailscale-remote-login': 'user@example.com', 'x-forwarded-for': '100.1.2.3' }
    ...
    assert.equal(row.login, 'user@example.com')
    assert.equal(row.agent, 'DSH Dock app')
