...
describe('relay: proxy port up', () => {
  ...
  it('splices bytes through untouched, headers included', async () => {
    const res = await fetchRaw(relayPort, '/dsh/api/x', { headers: { 'x-forwarded-for': '100.1.2.3', 'tailscale-user-login': 'user@example.com' } })
    ...
    assert.equal(seen.at(-1).headers['tailscale-user-login'], 'user@example.com')
