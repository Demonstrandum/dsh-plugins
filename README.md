...
a fresh machine (fork build, plugins, Dock app, Tailscale remote).
...
- `<private-plugin>` — per-chat <private> Language kernels
  ...
  inline to the user, `.wl` source written beside each PNG) / `<private>_symbol` /
  `<private>_lint` / `<private>_kernel_*` lifecycle tools. Host + browser halves;
